  ...
  <div class="dashboard">
    ...
    <table>
      ...
      <tr v-for="(character, key) in characters" :key="key">
        ...
        <td>
          <button type="button" @click="deleteCharacter(character.id)">削除</button>
        </td>
      </tr>
    </table>
  </div>
...
export default {
  ...
  methods: {
    refresh: async function() {
      const res = await axios.get("http://localhost:8080/api/v1/characters");
    ...
    deleteCharacter: async function(id) {
      await axios.delete("http://localhost:8080/api/v1/characters/" + id);
      await this.refresh();
    }
  }
};
